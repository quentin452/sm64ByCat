...
extern Gfx *geo_exec_flying_carpet_timer_update(s32 callContext, struct GraphNode *node,
                                                UNUSED f32 mtx[4][4]);
extern Gfx *geo_exec_flying_carpet_create(s32 callContext, struct GraphNode *node,
                                          UNUSED f32 mtx[4][4]);
...
#define gLoadBlockTexture(dl, width, height, format, image)                                            \
    {                                                                                                  \
        ...
        gDPTileSync((dl));                                                                             \
        gDPSetTile((dl), (format), G_IM_SIZ_16b, 0, 0, G_TX_LOADTILE, 0, G_TX_NOMIRROR, G_TX_NOMASK,   \
                   G_TX_NOLOD, G_TX_NOMIRROR, G_TX_NOMASK, G_TX_NOLOD);                                \
        gDPLoadSync((dl));                                                                             \
