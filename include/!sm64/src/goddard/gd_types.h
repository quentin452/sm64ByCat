...
struct ObjShape {
    ...
    /* 0x48 */ s32
        gdDls[3]; // gd dl number for each frame buffer (??) [0, 1]; and an offset for GdDl->dlptr [2]
...
struct ObjFace {
    ...
    /* 0x30 */ s32 vtxCount;
    /* 0x34 */ struct ObjVertex
        *vertices[4]; // these can also be s32 indices? which are then replaced by `find_thisface_verts`
...
struct ObjAnimator {
    ...
    /* 0x18 */ struct ObjGroup
        *animdata; // animation data? a group, but the link points to something weird..
    /* 0x1C */ u8 pad1C[0x4];
    /* 0x20 */ s32 unk20;
    /* 0x24 */ f32 unk24;
    /* 0x28 */ f32 unk28;
...
struct ObjUnk200000 {
    ...
    /* 0x30 */ struct ObjVertex
        *unk30; // not sure; guessing for Unknown801781DC; 30 and 34 could switch with ObjZone
