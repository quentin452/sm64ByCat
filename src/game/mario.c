...
void update_mario_inputs(struct MarioState *m) {
    ...
    /* Moonjump cheat */
    while (Cheats.MoonJump == true && Cheats.EnableCheats == true
...
u32 update_and_return_cap_flags(struct MarioState *m) {
    ...
    if (m->capTimer > 0) {
        ...
        if ((m->capTimer <= 60)
            ...
            if (Cheats.EnableCheats && Cheats.ForeverCap) {
                m->capTimer -= 0;
            } else {
                m->capTimer -= 1;
            }
