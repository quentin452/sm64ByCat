...
static void gfx_sdl_init(const char *window_title) {
    ...
    SDL_GL_SetAttribute(SDL_GL_CONTEXT_MAJOR_VERSION,
                        2); // These attributes allow for hardware acceleration on RPis.
    ...
    wnd = SDL_CreateWindow(window_title, xpos, ypos, configWindow.w, configWindow.h,
...
static void gfx_sdl_swap_buffers_end(void) {
    ...
    // Sleep off any remaining time to make the main loop iteration be called at a consistent frane
    // rate. We do this after our swap, because it actually will take the time to swap into account.
