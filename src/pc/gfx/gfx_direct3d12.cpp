...
#include <!sm64/src/pc/gfx/gfx_direct3d12_guids.h>
...
static struct {
    ...
    std::vector<std::pair<struct TextureHeap *, uint8_t>>
        texture_heap_allocations_to_reclaim_at_end_of_frame;
...
static D3D12_CPU_DESCRIPTOR_HANDLE get_cpu_descriptor_handle(ComPtr<ID3D12DescriptorHeap> &heap) {
    ...
    // Per MS documentation "User-defined types can be returned by value from global functions and
    // static member functions"... "Otherwise, the caller assumes the responsibility of allocating
    // memory and passing a pointer for the return value as the first argument". The method here is a
    // non-static member function, and hence we need to pass the address to the return value as a
    // parameter. MinGW32 has the same issue.
...
get_resource_allocation_info(const D3D12_RESOURCE_DESC *resource_desc) {
#ifdef __MINGW32__
    // See get_cpu_descriptor_handle
    ...
    void (STDMETHODCALLTYPE ID3D12Device::*fun)(D3D12_RESOURCE_ALLOCATION_INFO * out, UINT visibleMask,
        ...
        (void (STDMETHODCALLTYPE ID3D12Device::*)(D3D12_RESOURCE_ALLOCATION_INFO * out,
                                                  UINT visibleMask, UINT numResourceDescs,
                                                  const D3D12_RESOURCE_DESC *pResourceDescs)) fn;
...
static void gfx_direct3d12_shader_get_info(struct ShaderProgram *prg, uint8_t *num_inputs,
                                           bool used_textures[2]) {
...
static void gfx_direct3d12_upload_texture(const uint8_t *rgba32_buf, int width, int height) {
    ...
    ThrowIfFailed(d3d.device->CreatePlacedResource(
        found_heap->heap.Get(), heap_offset * alloc_info.SizeInBytes, &texture_desc,
    ...
    CD3DX12_RESOURCE_BARRIER barrier =
        CD3DX12_RESOURCE_BARRIER::Transition(texture_resource.Get(), D3D12_RESOURCE_STATE_COPY_DEST,
                                             D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
    d3d.command_list->ResourceBarrier(1, &barrier);

    d3d.upload_heaps_in_flight.push_back(
        std::make_pair((size_t) upload_buffer_size, std::move(upload_heap)));
    ...
    if (td.resource.Get() != nullptr) {
        ...
        d3d.texture_heap_allocations_to_reclaim_at_end_of_frame.push_back(
            std::make_pair(td.heap, td.heap_offset));
...
static void gfx_direct3d12_set_sampler_parameters(int tile, bool linear_filter, uint32_t cms,
                                                  uint32_t cmt) {
...
static void gfx_direct3d12_draw_triangles(float buf_vbo[], size_t buf_vbo_len,
    ...
    if (d3d.must_reload_pipeline) {
        ComPtr<ID3D12PipelineState> &pipeline_state = d3d.pipeline_states[PipelineDesc{
            prg->shader_id, d3d.depth_test, d3d.depth_mask, d3d.zmode_decal, 0 }];
        if (pipeline_state.Get() == nullptr) {
            ...
            for (int i = 0; i < prg->num_inputs; i++) {
                DXGI_FORMAT format = (prg->shader_id & SHADER_OPT_ALPHA)
                                         ? DXGI_FORMAT_R32G32B32A32_FLOAT
            ...
            ThrowIfFailed(
                d3d.device->CreateGraphicsPipelineState(&desc, IID_PPV_ARGS(&pipeline_state)));
        }
    ...
    if ((prg->shader_id & (SHADER_OPT_ALPHA | SHADER_OPT_NOISE))
        == (SHADER_OPT_ALPHA | SHADER_OPT_NOISE)) {
        d3d.command_list->SetGraphicsRootConstantBufferView(root_param_index++,
                                                            d3d.noise_cb->GetGPUVirtualAddress());
    }
    ...
    for (int i = 0; i < 2; i++) {
        if (prg->used_textures[i]) {
            ...
            if (td.last_frame_counter != d3d.frame_counter) {
                ...
                CD3DX12_CPU_DESCRIPTOR_HANDLE srv_handle(get_cpu_descriptor_handle(d3d.srv_heap),
                                                         d3d.srv_pos++, d3d.srv_descriptor_size);
                d3d.device->CreateShaderResourceView(td.resource.Get(), &srv_desc, srv_handle);
            }

            CD3DX12_GPU_DESCRIPTOR_HANDLE srv_gpu_handle(get_gpu_descriptor_handle(d3d.srv_heap),
                                                         td.descriptor_index, d3d.srv_descriptor_size);
            ...
            CD3DX12_GPU_DESCRIPTOR_HANDLE sampler_gpu_handle(
                get_gpu_descriptor_handle(d3d.sampler_heap), td.sampler_parameters,
                d3d.sampler_descriptor_size);
...
static void gfx_direct3d12_start_frame(void) {
    ...
    CD3DX12_RESOURCE_BARRIER barrier = CD3DX12_RESOURCE_BARRIER::Transition(
        d3d.render_targets[d3d.frame_index].Get(), D3D12_RESOURCE_STATE_PRESENT,
...
static void create_render_target_views(void) {
    ...
    for (UINT i = 0; i < 2; i++) {
        ThrowIfFailed(
            d3d.swap_chain->GetBuffer(i, IID_ID3D12Resource, (void **) &d3d.render_targets[i]));
...
static void create_depth_buffer(void) {
    ...
    ThrowIfFailed(d3d.device->CreateCommittedResource(
        &hp, D3D12_HEAP_FLAG_NONE, &rd, D3D12_RESOURCE_STATE_DEPTH_WRITE, &depth_optimized_cv,
        IID_PPV_ARGS(&d3d.depth_stencil_buffer)));
...
static void gfx_direct3d12_on_resize(void) {
    if (d3d.render_targets[0].Get() != nullptr) {
        ...
        ThrowIfFailed(d3d.swap_chain->ResizeBuffers(
...
static void gfx_direct3d12_init(void) {
    ...
    d3d.D3D12CreateDevice =
        (PFN_D3D12_CREATE_DEVICE) GetProcAddress(d3d.d3d12_module, "D3D12CreateDevice");
#if DEBUG_D3D
    ...
    }
    {
        ...
        ThrowIfFailed(
            d3d.device->CreateCommandQueue(&queue_desc, IID_PPV_ARGS(&d3d.copy_command_queue)));
    }
    ...
    // Create render target views
    {
        ...
        d3d.rtv_descriptor_size =
            d3d.device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
    ...
    // Create SRV heap for texture descriptors
    {
        ...
        d3d.srv_descriptor_size =
            d3d.device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    }
    ...
    // Create sampler heap and descriptors
    {
        ...
        ThrowIfFailed(
            d3d.device->CreateDescriptorHeap(&sampler_heap_desc, IID_PPV_ARGS(&d3d.sampler_heap)));
        d3d.sampler_descriptor_size =
            d3d.device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER);
        ...
        for (int linear_filter = 0; linear_filter < 2; linear_filter++) {
            for (int cms = 0; cms < 3; cms++) {
                for (int cmt = 0; cmt < 3; cmt++) {
                    ...
                    sampler_desc.Filter = linear_filter ? D3D12_FILTER_MIN_MAG_MIP_LINEAR
                    ...
                    d3d.device->CreateSampler(
                        &sampler_desc, CD3DX12_CPU_DESCRIPTOR_HANDLE(sampler_handle, pos++,
                                                                     d3d.sampler_descriptor_size));
                }
            }
        }
    ...
    ThrowIfFailed(d3d.device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT,
                                                     IID_PPV_ARGS(&d3d.command_allocator)));
    ThrowIfFailed(d3d.device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_COPY,
                                                     IID_PPV_ARGS(&d3d.copy_command_allocator)));

    ThrowIfFailed(d3d.device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT,
                                                d3d.command_allocator.Get(), nullptr,
                                                IID_PPV_ARGS(&d3d.command_list)));
    ThrowIfFailed(d3d.device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_COPY,
                                                d3d.copy_command_allocator.Get(), nullptr,
                                                IID_PPV_ARGS(&d3d.copy_command_list)));
...
static void gfx_direct3d12_end_frame(void) {
    ...
    CD3DX12_RESOURCE_BARRIER barrier = CD3DX12_RESOURCE_BARRIER::Transition(
        d3d.render_targets[d3d.frame_index].Get(), D3D12_RESOURCE_STATE_RENDER_TARGET,
...
static void gfx_direct3d12_finish_render(void) {
    ...
    for (std::pair<struct TextureHeap *, uint8_t> &item :
         d3d.texture_heap_allocations_to_reclaim_at_end_of_frame) {
    ...
    // long)(t0.QuadPart - d3d.qpc_init), (int)(t1.QuadPart - t0.QuadPart), (int)(t2.QuadPart -
    // t0.QuadPart), (unsigned long long)(t2.QuadPart - d3d.qpc_init),
    // d3d.pending_frame_stats.rbegin()->first, stats.PresentCount, stats.SyncRefreshCount, (unsigned
    // long long)(stats.SyncQPCTime.QuadPart - d3d.qpc_init));
}

} // namespace

struct GfxRenderingAPI gfx_direct3d12_api = {
    gfx_direct3d12_z_is_from_0_to_1, gfx_direct3d12_unload_shader,
    gfx_direct3d12_load_shader,      gfx_direct3d12_create_and_load_new_shader,
    gfx_direct3d12_lookup_shader,    gfx_direct3d12_shader_get_info,
    gfx_direct3d12_new_texture,      gfx_direct3d12_select_texture,
    gfx_direct3d12_upload_texture,   gfx_direct3d12_set_sampler_parameters,
    gfx_direct3d12_set_depth_test,   gfx_direct3d12_set_depth_mask,
    gfx_direct3d12_set_zmode_decal,  gfx_direct3d12_set_viewport,
    gfx_direct3d12_set_scissor,      gfx_direct3d12_set_use_alpha,
    gfx_direct3d12_draw_triangles,   gfx_direct3d12_init,
    gfx_direct3d12_on_resize,        gfx_direct3d12_start_frame,
    gfx_direct3d12_end_frame,        gfx_direct3d12_finish_render
};
