...
static bool elf_get_section_range(uint8_t *file, const char *searched_name, uint32_t *address,
                                  uint32_t *offset, uint32_t *size, uint32_t *section_index) {
    ...
    for (int i = 0; i < u16be(ehdr->e_shnum); i++) {
        ...
        Elf32_Shdr *str_shdr = (Elf32_Shdr *) (file + u32be(ehdr->e_shoff)
                                               + u16be(ehdr->e_shstrndx) * u16be(ehdr->e_shentsize));
...
int main(int argc, char *argv[]) {
    ...
    if (elf_get_section_range(file, ".rodata", &rodata_address, &rodata_offset, &rodata_size,
                              &rodata_index)) {
        fprintf(
            stderr,
            ".rodata section found, please put everything in .data instead (non-const variables)\n");
        exit(1);
    }

    uint32_t symtab_address, symtab_offset, symtab_size, symtab_index;
    if (!elf_get_section_range(file, ".symtab", &symtab_address, &symtab_offset, &symtab_size,
                               &symtab_index)) {
    ...
    if (!elf_get_section_range(file, ".strtab", &strtab_address, &strtab_offset, &strtab_size,
                               &strtab_index)) {
    ...
    for (uint32_t i = 0; i < symtab_size / sizeof(Elf32_Sym); i++) {
        ...
        printf("%08x\t%08x\t%02x\t%02x\t%02x\t%s\n", u32be(symbol->st_value), u32be(symbol->st_size),
               symbol->st_info, symbol->st_other, u16be(symbol->st_shndx), name);
#endif
    ...
    if (elf_get_section_range(file, ".mdebug", &mdebug_address, &mdebug_offset, &mdebug_size,
                              &mdebug_index)) {
        ...
        for (uint32_t i = 0; i < u32be(symbolic_header->ifdMax); i++) {
            FileDescriptorTable *fdt =
                (FileDescriptorTable *) (file + u32be(symbolic_header->cbFdOffset)
                                         + i * sizeof(FileDescriptorTable));
            ...
            for (uint32_t j = 0; j < u32be(fdt->csym); j++) {
                ...
                if (st == stStatic || st == stGlobal) {
                    // Right now just assume length 8 since it's quite much work to extract the real
                    // size
    ...
    if (new_size + 16 <= data_size) {
        // There seems to be more than 16 bytes padding or non-identified data, so abort and take the
        // original size
        new_size = data_size;
    } else {
