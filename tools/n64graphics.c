        ...
        case 4: // red, green, blue, alpha
            ...
            for (int j = 0; j < h; j++) {
                for (int i = 0; i < w; i++) {
                    int idx = j * w + i;
                    int sum =
                        data[channels * idx] + data[channels * idx + 1] + data[channels * idx + 2];
...
static void print_usage(void) {
    ERROR("Usage: n64graphics -e/-i BIN_FILE -g IMG_FILE [-p PAL_FILE] [-o BIN_OFFSET] [-P PAL_OFFSET] "
          "[-f FORMAT] [-c CI_FORMAT] [-w WIDTH] [-h HEIGHT] [-V]\n"
          "\n"
          ...
          " -f FORMAT     texture format: rgba16, rgba32, ia1, ia4, ia8, ia16, i4, i8, ci4, ci8 "
          "(default: %s)\n"
          ...
          format2str(&default_config.format), default_config.width, default_config.height,
...
static int valid_config(const graphics_config *config) {
    ...
    if (config->format.format == IMG_FORMAT_CI) {
        if (!config->pal_filename || (config->pal_format.depth != 16)
            || (config->pal_format.format != IMG_FORMAT_RGBA
                && config->pal_format.format != IMG_FORMAT_IA)) {
            return 0;
        }
    }
    return 1;
            ...
            case IMG_FORMAT_CI: {
                ...
                switch (config.pal_format.format) {
                    case IMG_FORMAT_RGBA:
                        imgr = png2rgba(config.img_filename, &config.width, &config.height);
                        raw16_length =
                            rgba2raw(raw16, imgr, config.width, config.height, config.pal_format.depth);
                        break;
                    case IMG_FORMAT_IA:
                        imgi = png2ia(config.img_filename, &config.width, &config.height);
                        raw16_length =
                            ia2raw(raw16, imgi, config.width, config.height, config.pal_format.depth);
                        break;
                    default:
        ...
        switch (config.format.format) {
            ...
            case IMG_FORMAT_CI: {
                ...
                INFO("Extracting %s offset 0x%X, pal.offset 0x%0X, pal.format %s\n",
                     format2str(&config.format), config.bin_offset, config.pal_offset,
                     format2str(&config.pal_format));
