...
typedef enum { IMG_FORMAT_CI } img_format;
...
static void print_usage(void) {
    ERROR("Usage: n64graphics_ci -e/-i BIN_FILE -g PNG_FILE [-o offset] [-f FORMAT] [-w WIDTH] [-h "
          "HEIGHT] [-V]\n"
          "\n"
          ...
          format2str(default_config.format, default_config.depth), default_config.width,
    ...
    if (config.mode == MODE_IMPORT) {
        ...
        switch (config.format) {
            case IMG_FORMAT_CI:
                ...
                length =
                    rgba2rawci(raw, pal, &pal_len, imgr, config.width, config.height, config.depth);
