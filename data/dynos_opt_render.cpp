...
#define DYNOS_TEXT_DYNOS_MENU                                                                          \
    { "DYNOS MENU", NULL }
#define DYNOS_TEXT_A                                                                                   \
    { "([A]) >", NULL }
#define DYNOS_TEXT_OPEN_LEFT                                                                           \
    { "[Z] DynOS", NULL }
#define DYNOS_TEXT_CLOSE_LEFT                                                                          \
    { "[Z] Return", NULL }
#define DYNOS_TEXT_OPTIONS_MENU                                                                        \
    { "OPTIONS", NULL }
#define DYNOS_TEXT_DISABLED                                                                            \
    { "Disabled", NULL }
#define DYNOS_TEXT_ENABLED                                                                             \
    { "Enabled", NULL }
#define DYNOS_TEXT_NONE                                                                                \
    { "NONE", NULL }
#define DYNOS_TEXT_DOT_DOT_DOT                                                                         \
    { "...", NULL }
#define DYNOS_TEXT_OPEN_RIGHT                                                                          \
    { "[R] Options", NULL }
#define DYNOS_TEXT_CLOSE_RIGHT                                                                         \
    { "[R] Return", NULL }
...
static void PrintString(const Label &aLabel, s32 aX, s32 aY, u32 aFrontColorRGBA, u32 aBackColorRGBA,
                        bool aAlignLeft) {
    const u8 *_Str64 =
        (aLabel.second ? aLabel.second : DynOS_String_Convert(aLabel.first.begin(), false));

    gSPDisplayList(gDisplayListHead++, dl_ia_text_begin);
    if ((aBackColorRGBA & 0xFF) != 0) {
        gDPSetEnvColor(gDisplayListHead++, ((aBackColorRGBA >> 24) & 0xFF),
                       ((aBackColorRGBA >> 16) & 0xFF), ((aBackColorRGBA >> 8) & 0xFF),
                       ((aBackColorRGBA >> 0) & 0xFF));
        if (aAlignLeft) {
            RenderString(_Str64, GFX_DIMENSIONS_FROM_LEFT_EDGE(aX) + 1, aY - 1);
        } else {
            RenderString(_Str64, GFX_DIMENSIONS_FROM_RIGHT_EDGE(aX + DynOS_String_Width(_Str64) - 1),
                         aY - 1);
        }
    }
    if ((aFrontColorRGBA & 0xFF) != 0) {
        gDPSetEnvColor(gDisplayListHead++, ((aFrontColorRGBA >> 24) & 0xFF),
                       ((aFrontColorRGBA >> 16) & 0xFF), ((aFrontColorRGBA >> 8) & 0xFF),
                       ((aFrontColorRGBA >> 0) & 0xFF));
        if (aAlignLeft) {
...
static void PrintBox(s32 aX, s32 aY, s32 aWidth, s32 aHeight, u32 aColorRGBA, bool aAlignLeft) {
    if ((aColorRGBA && 0xFF) != 0) {
        ...
        if (!_Matrix)
            return;
        if (aAlignLeft) {
            create_dl_translation_matrix(MENU_MTX_PUSH, GFX_DIMENSIONS_FROM_LEFT_EDGE(aX), aY + aHeight,
                                         0);
        } else {
            create_dl_translation_matrix(MENU_MTX_PUSH, GFX_DIMENSIONS_FROM_RIGHT_EDGE(aX + aWidth),
                                         aY + aHeight, 0);
        }
        guScale(_Matrix, (f32) aWidth / 130.f, (f32) aHeight / 80.f, 1.f);
        gSPMatrix(gDisplayListHead++, VIRTUAL_TO_PHYSICAL(_Matrix),
                  G_MTX_MODELVIEW | G_MTX_MUL | G_MTX_NOPUSH);
        gDPSetEnvColor(gDisplayListHead++, ((aColorRGBA >> 24) & 0xFF), ((aColorRGBA >> 16) & 0xFF),
                       ((aColorRGBA >> 8) & 0xFF), ((aColorRGBA >> 0) & 0xFF));
...
#define get_dec_number(n)                                                                              \
    { "", DynOS_String_Convert(IntToString("%d", n), false) }
#define get_hex_number(n)                                                                              \
    { "", DynOS_String_Convert(IntToString("%04X", n), false) }
#define get_level(opt)                                                                                 \
    { "", DynOS_Level_GetName(DynOS_Level_GetList()[*opt->mChoice.mIndex], true, true) }
#define get_star(opt)                                                                                  \
    {                                                                                                  \
        "", DynOS_Level_GetActName(DynOS_Level_GetList()[DynOS_Opt_GetValue("dynos_warp_level")],      \
                                   *opt->mChoice.mIndex + 1, true, true)                               \
    }
#define get_param(opt)                                                                                 \
    {                                                                                                  \
        DynOS_Warp_GetParamName(DynOS_Level_GetList()[DynOS_Opt_GetValue("dynos_warp_level")],         \
                                *opt->mChoice.mIndex),                                                 \
            NULL                                                                                       \
    }
...
static s32 GetCurrentOptionCount(DynosOption *aCurrentOpt) {
    s32 _Count = 0;
    while (aCurrentOpt->mPrev) {
        aCurrentOpt = aCurrentOpt->mPrev;
    }
    while (aCurrentOpt) {
        aCurrentOpt = aCurrentOpt->mNext;
        _Count++;
    }
    return _Count;
}
...
static void DynOS_Opt_DrawOption(DynosOption *aOpt, DynosOption *aCurrentOpt, s32 aY) {
    ...
    if (aOpt == aCurrentOpt) {
        ...
        PrintBox(OFFSET_FROM_LEFT_EDGE - 4, aY - 2,
                 GFX_DIMENSIONS_FROM_RIGHT_EDGE(OFFSET_FROM_RIGHT_EDGE)
                     - GFX_DIMENSIONS_FROM_LEFT_EDGE(OFFSET_FROM_LEFT_EDGE) + 8,
                 20, COLOR_SELECT_BOX + _Alpha, 1);
    }
    ...
    switch (aOpt->mType) {
        case DOPT_TOGGLE: {
            if (*aOpt->mToggle.mTog) {
                PrintString(DYNOS_TEXT_ENABLED, OFFSET_FROM_RIGHT_EDGE, aY, COLOR_ENABLED, COLOR_BLACK,
                            0);
            } else {
                PrintString(DYNOS_TEXT_DISABLED, OFFSET_FROM_RIGHT_EDGE, aY, COLOR_DISABLED,
                            COLOR_BLACK, 0);
            }
        } break;

        case DOPT_CHOICE: {
            PrintString(get_choice(aOpt), OFFSET_FROM_RIGHT_EDGE, aY,
                        aOpt == aCurrentOpt ? COLOR_SELECT : COLOR_WHITE, COLOR_BLACK, 0);
        } break;

        case DOPT_CHOICELEVEL: {
            PrintString(get_level(aOpt), OFFSET_FROM_RIGHT_EDGE, aY,
                        aOpt == aCurrentOpt ? COLOR_SELECT : COLOR_WHITE, COLOR_BLACK, 0);
        } break;
        ...
        case DOPT_CHOICEAREA: {
            ...
            if (DynOS_Level_GetWarpEntry(_Level, _Area)) {
                PrintString({ "", _Name }, OFFSET_FROM_RIGHT_EDGE, aY,
                            aOpt == aCurrentOpt ? COLOR_SELECT : COLOR_WHITE, COLOR_BLACK, 0);
            } else {
        ...
        case DOPT_CHOICESTAR: {
            s32 _Course =
                DynOS_Level_GetCourse(DynOS_Level_GetList()[DynOS_Opt_GetValue("dynos_warp_level")]);
            if (_Course >= COURSE_MIN && _Course <= COURSE_STAGES_MAX) {
                PrintString(get_star(aOpt), OFFSET_FROM_RIGHT_EDGE, aY,
                            aOpt == aCurrentOpt ? COLOR_SELECT : COLOR_WHITE, COLOR_BLACK, 0);
            }
        } break;
        ...
        case DOPT_CHOICEPARAM: {
            PrintString(get_param(aOpt), OFFSET_FROM_RIGHT_EDGE, aY,
                        aOpt == aCurrentOpt ? COLOR_SELECT : COLOR_WHITE, COLOR_BLACK, 0);
        } break;
#endif

        case DOPT_SCROLL: {
            s32 _Width = (s32)(SCROLL_BAR_SIZE * (f32)(*aOpt->mScroll.mValue - aOpt->mScroll.mMin)
                               / (f32)(aOpt->mScroll.mMax - aOpt->mScroll.mMin));
            PrintString(get_dec_number(*aOpt->mScroll.mValue), OFFSET_FROM_RIGHT_EDGE, aY,
                        aOpt == aCurrentOpt ? COLOR_SELECT : COLOR_WHITE, COLOR_BLACK, 0);
            PrintBox(OFFSET_FROM_RIGHT_EDGE + 28, aY + 4, SCROLL_BAR_SIZE + 2, 8, COLOR_DARK_GRAY, 0);
            PrintBox(OFFSET_FROM_RIGHT_EDGE + 29 + SCROLL_BAR_SIZE - _Width, aY + 5, _Width, 6,
                     aOpt == aCurrentOpt ? COLOR_SELECT : COLOR_WHITE, 0);
        } break;
        ...
        case DOPT_BIND: {
            for (s32 i = 0; i != 3; ++i) {
                ...
                if (aOpt == aCurrentOpt && i == aOpt->mBind.mIndex) {
                    if (sBindingState != 0) {
                        PrintString(DYNOS_TEXT_DOT_DOT_DOT, OFFSET_FROM_RIGHT_EDGE + (2 - i) * 36, aY,
                                    COLOR_SELECT, COLOR_BLACK, 0);
                    } else if (_Bind == VK_INVALID) {
                        PrintString(DYNOS_TEXT_NONE, OFFSET_FROM_RIGHT_EDGE + (2 - i) * 36, aY,
                                    COLOR_SELECT, COLOR_BLACK, 0);
                    } else {
                        PrintString(get_hex_number(_Bind), OFFSET_FROM_RIGHT_EDGE + (2 - i) * 36, aY,
                                    COLOR_SELECT, COLOR_BLACK, 0);
                    }
                } else {
                    if (_Bind == VK_INVALID) {
...
void DynOS_Opt_DrawMenu(DynosOption *aCurrentOption, DynosOption *aCurrentMenu,
    ...
    gDPSetEnvColor(gDisplayListHead++, 255, 255, 255, 255);
    if (!_Title.second)
        _Title.second = DynOS_String_Convert(_Title.first.begin(), false);
    print_hud_lut_string(HUD_LUT_GLOBAL, (SCREEN_WIDTH / 2 - DynOS_String_Length(_Title.second) * 6),
                         40, _Title.second);
    ...
    if (_OptCount > 7) {
        ...
        s32 _Y =
            37
            + (134 - _Height) * (1.f - MAX(0.f, MIN(1.f, (f32)(_OptIndex - 3) / (f32)(_OptCount - 6))));
