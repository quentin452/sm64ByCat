...
static bool DynOS_Opt_ControllerIsKeyDown(s32 aCont, s32 aKey) {
    ...
    // Game Controller
        ...
        if (_Button < SDL_CONTROLLER_BUTTON_MAX) {
            return SDL_GameControllerGetButton(SDL_GameControllerOpen(aCont - 1),
        ...
        if (_Axis < SDL_CONTROLLER_AXIS_MAX * 2) {
            s32 _AxisValue = SDL_GameControllerGetAxis(SDL_GameControllerOpen(aCont - 1),
                                                       SDL_GameControllerAxis(_Axis / 2));
            if (_Axis & 1)
                return (_AxisValue < (SHRT_MIN / 2));
            else
                return (_AxisValue > (SHRT_MAX / 2));
        }
    }
    // Invalid
    return false;
    ...
    if (aOpt->mType == DOPT_BIND) {
        ...
        for (s32 _Cont = 0; _Cont < MAX_CONTS; ++_Cont)
            for (s32 _Bind = 0; _Bind < 3; ++_Bind) {
                pad->button |=
                    aOpt->mBind.mMask * DynOS_Opt_ControllerIsKeyDown(_Cont, aOpt->mBind.mBinds[_Bind]);
            }
    }
    return false;
}
