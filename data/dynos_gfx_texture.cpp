...
u8 *DynOS_Gfx_TextureConvertToRGBA32(const u8 *aData, u64 aLength, s32 aFormat, s32 aSize,
    ...
    switch ((aFormat << 8) | aSize) {
        case ((G_IM_FMT_RGBA << 8) | G_IM_SIZ_16b):
            return RGBA16_RGBA32(aData, aLength);
        case ((G_IM_FMT_RGBA << 8) | G_IM_SIZ_32b):
            return RGBA32_RGBA32(aData, aLength);
        case ((G_IM_FMT_IA << 8) | G_IM_SIZ_4b):
            return IA4_RGBA32(aData, aLength);
        case ((G_IM_FMT_IA << 8) | G_IM_SIZ_8b):
            return IA8_RGBA32(aData, aLength);
        case ((G_IM_FMT_IA << 8) | G_IM_SIZ_16b):
            return IA16_RGBA32(aData, aLength);
        case ((G_IM_FMT_CI << 8) | G_IM_SIZ_4b):
            return CI4_RGBA32(aData, aLength, aPalette);
        case ((G_IM_FMT_CI << 8) | G_IM_SIZ_8b):
            return CI8_RGBA32(aData, aLength, aPalette);
        case ((G_IM_FMT_I << 8) | G_IM_SIZ_4b):
            return I4_RGBA32(aData, aLength);
        case ((G_IM_FMT_I << 8) | G_IM_SIZ_8b):
            return I8_RGBA32(aData, aLength);
    }
    return NULL;
}
...
static void DynOS_Gfx_UploadTexture(DataNode<TexData> *aNode, GRAPI *aGfxRApi, s32 aTile, s32 aTexId) {
    ...
    aGfxRApi->upload_texture(aNode->mData->mRawData.begin(), aNode->mData->mRawWidth,
                             aNode->mData->mRawHeight);
...
struct THN {
    struct THN *mNext;
    const void
        *mAddr; // Contains the pointer to the DataNode<TexData> struct, NOT the actual texture data
    u8 mFmt, mSiz;
    s32 mTexId;
    u8 mCms, mCmt;
    bool mLInf;
...
static bool DynOS_Gfx_CacheTexture(THN **aOutput, DataNode<TexData> *aNode, s32 aTile, GRAPI *aGfxRApi,
                                   THN **aHashMap, THN *aPool, u32 *aPoolPos, u32 aPoolSize) {
...
static bool DynOS_Gfx_ImportTexture_Typed(THN **aOutput, void *aPtr, s32 aTile, GRAPI *aGfxRApi,
                                          THN **aHashMap, THN *aPool, u32 *aPoolPos, u32 aPoolSize) {
    DataNode<TexData> *_Node = DynOS_Gfx_RetrieveNode(aPtr);
    if (_Node) {
        if (!DynOS_Gfx_CacheTexture(aOutput, _Node, aTile, aGfxRApi, aHashMap, aPool, aPoolPos,
                                    aPoolSize)) {
...
bool DynOS_Gfx_ImportTexture(void **aOutput, void *aPtr, s32 aTile, void *aGfxRApi, void **aHashMap,
    ...
    return DynOS_Gfx_ImportTexture_Typed((THN **) aOutput, (void *) aPtr, (s32) aTile,
                                         (GRAPI *) aGfxRApi, (THN **) aHashMap, (THN *) aPool,
                                         (u32 *) aPoolPos, (u32) aPoolSize);
}
